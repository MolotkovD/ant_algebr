...
<center>
<img src="https://latex.codecogs.com/svg.image?\bg_white&space;P_{ij,k}(t)&space;=&space;\frac{[\tau&space;_{ij}(t)]^{\alpha}[n_{ij}]^{\beta}}{\sum[\tau&space;_{ij}(t)]^\alpha&space;[n_{ij}]^\beta&space;}&space;" title="\bg_white P_{ij,k}(t) = \frac{[\tau _{ij}(t)]^{\alpha}[n_{ij}]^{\beta}}{\sum[\tau _{ij}(t)]^\alpha [n_{ij}]^\beta } " />
</center>
...
$$
τ_{ij}(t+1)=(1-p)τ_{ij}(t)+\sum_{
\begin{subarray}{l}
k \in{used(i,j)}
\end{subarray}}\frac{Q}{L_k(t)}
$$

где $ρ$ – скорость испарения, $t$ – номер итерации, $Lk(t)$ – цена текущего решения для $k$-ого муравья, а $Q$ – параметр, имеющий значение порядка цены оптимального решения, то есть $\frac{Q}{Lk(t)}$ – феромон, откладываемый $k$-ым муравьём, использующим ребро $(i, j)$.
